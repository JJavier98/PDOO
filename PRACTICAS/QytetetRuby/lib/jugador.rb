...
module ModeloQytetet
	...
	class Jugador
		attr_accessor :encarcelado, :casillaActual, :saldo, :propiedades, :cartaLibertad, :nombre
    
		def initialize n, enc = false, sald = 0, carta = nil, casilla = nil, prop = 0
			@encarcelado = enc
			@nombre = n
			@saldo = sald
			@cartaLibertad = carta
			@casillaActual = casilla
			@propiedades = Array.new(prop)
		end
		...
		def actualizar_posicion casilla
			...
			if(@casillaActual.soy_edificable())
				...
				pagar_impuestos(@casillaActual.coste)

...
module ModeloQytetet
	...
	class Qytetet
		...
		def inicializar_cartas_sorpresa()
			@mazo << Sorpresa.new("Te has convertido en un jugador ESPECULADOR", 3000, TipoSorpresa::CONVERTIRME)
			@mazo << Sorpresa.new("Te has convertido en un jugador ESPECULADOR",5000, TipoSorpresa::CONVERTIRME)
			...
			if (@cartaActual.tipo == TipoSorpresa::PAGARCOBRAR)
				...
				while (i < @jugadores.length())
					if(@jugadores[i] == @jugadorActual)
						...
						@jugadores[i] = @jugadorActual = espec
					end
					i = i+1
		...
		def jugar
			...
			if(nueva_casilla.tipo != TipoCasilla::CALLE)
				puts "Caes en la casilla:\n   *númeroCasilla: #{nueva_casilla.numeroCasilla}" +
					   "\n   *Tipo: #{nueva_casilla.tipo}\n"
			elsif(nueva_casilla.tipo == TipoCasilla::CALLE && nueva_casilla.tengo_propietario())
					puts "Caes en la casilla:\n   *númeroCasilla: #{nueva_casilla.numeroCasilla}" +
						  "\n   *Tipo: #{nueva_casilla.tipo}" +
						  "\n   *Propietario: #{nueva_casilla.titulo.propietario.nombre}" + "\n"
			end
